...
# `duration-[var(--duration-normal)]` is the *correct* form and contains the bare
# name as a substring, so the match has to require the class to end right there.
...
# View files only: `views/<page>.tsx` and the folder form `views/<page>/index.tsx`.
# A page's sections live beside its view (obsidian/architecture/folder-structure.md)
# and are exactly where the client boundary is *supposed* to be, so recursing into
# src/views would flag every correctly-written leaf.
view_files="$(find src/views -maxdepth 1 -name '*.tsx' 2>/dev/null; find src/views -mindepth 2 -maxdepth 2 -name 'index.tsx' 2>/dev/null)"
report WARN '"use client" on a layout, page or view' \
  "Server Components by default — push the boundary down to a leaf component." \
  "$(grep -rln '"use client"' src/app/layout.tsx src/app/page.tsx $view_files 2>/dev/null)"
