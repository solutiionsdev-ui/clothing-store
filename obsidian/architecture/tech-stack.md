...
| `spring-text-engine` | `^0.1.5` | Scroll-aware spring text animation. **Currently unused** — see [[text-engine]] |
...
| Package | Version | Role |
|---------|---------|------|
| `three` | `^0.185.1` | WebGL renderer for the hero product model |
| `@types/three` | dev | Type definitions |

No `@react-three/fiber` and no `drei`: the scene is a single static model, and
...
> [!warning] There is now a WebGL scene in this project
> AGENTS.md hard rule #8 is live: a performance, jank or shipping-readiness
> request must invoke the **`optimize-3d-scene`** skill first and follow its
> order of fixes. See [[optimize-3d-scene]].
