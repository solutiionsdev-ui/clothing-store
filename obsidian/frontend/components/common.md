...
**How it works** — `vw`-based `html { font-size }` media queries in
`globals.css`. At each breakpoint's design base width the root font-size resolves
to 16px; between breakpoints it tracks the viewport.
...
> [!important] In this project the top rule is unbounded upwards
> The only desktop frame is 1440 wide, so the 1440 rule carries every width above
> it and the design keeps filling the viewport instead of being pinned and
> letterboxed. That makes the scale **pure CSS at every width**, so
> `<AdaptiveGrid />` is **not mounted** — a JS pass would only add a flash of
> unscaled layout on first paint. See [[decisions-log]] ADR-0024.

**When you would mount it again** — once a 1920 frame exists, bound the 1440 rule
at `max-width: 1920px`, add the 1920 breakpoint to `grid.config.ts`, and render
`<AdaptiveGrid />` in the root layout to drive the scale-up beyond it:
```tsx
import { AdaptiveGrid } from "@/components/common/grid";
```
Mount it once. Props: `baseWidth` (defaults to the largest breakpoint) and
`coef` (0–1 scale-up damping, default `0.6666`; `1` is fully proportional).
