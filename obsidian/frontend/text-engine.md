...
> [!note] Currently unused in this project
> The hero's copy reveals through the decode effect instead
> ([[components/ui|`<ScrambleText>`]], [[decisions-log]] ADR-0026) — the engine
> springs a *property* of a letter, which is not what that effect does. The
> package stays installed and this note stays canonical: the engine is still the
> tool for a prose or headline reveal, and hard rule #1 still routes that kind of
> text animation here.
...
The engine splits text into flex items and **drops the original spaces**, then
re-creates them with `column-gap`. Neither the default nor the obvious escape
hatch preserves the copy's measure:

- the default is `0.3` (em), which is tighter than a real space in most faces;
- `"inherit"` resolves to `normal` on a flex container — **no gap at all**, so
  the words run together.

Pass the face's own space advance in `em` — that is exactly the space the
engine removed. Measure it with `canvas.measureText(' ').width / fontSize`; for
IBM 3270, the display face here, it is 0.54em.

**Line breaks.** The container wraps, so a block with hand-set breaks in the
design should not be left to re-wrap. Give each designed line its own engine
instance (a `span` inside the real paragraph or heading) — that keeps the break
exactly where the frame put it, and keeps the semantic element intact.
...
or `type="toggle"` (snap), plus `start`/`end` trigger positions.
