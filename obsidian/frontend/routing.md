...
All layout and UI logic lives in `src/views/home/` (`HomeView` in `index.tsx`).
The view is a **Server Component**; client-only animation is isolated in leaf
components — see [[component-conventions]] hard rule #6. It renders the page
banner and `main`, and delegates the page's UI to the sections beside it in
`src/views/home/hero/`.
...
| Route | File | View |
|-------|------|------|
| `/` | `src/app/page.tsx` | `views/home/` → `HomeView` (hero section) |
...
- `src/app/layout.tsx` — the **root layout**. Holds the provider tree
  (`ScrollLayout` → `ReducedMotion` / `Cookie` → children),
